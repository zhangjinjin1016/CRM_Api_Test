## CRM 接口自动化测试项目
是一个前后端分离的 CRM 系统，核心模块包括合同管理、课程管理。  
本项目通过 **Postman + Newman**（工具级）和 **Python + requests + pytest**（代码级）两种方式实现接口自动化测试，并设计了 Jenkins + Newman 的持续集成方案，用于学习接口自动化技术并提升回归效率。
...
6. 设计了基于 Jenkins + Newman 的 CI 方案（流程图见 `/docs/ci_design.png`），可实现代码提交后自动触发测试并邮件发送报告。

## 技术亮点
- **接口关联**：自动提取登录 token 并传递给后续接口
- **多维度断言**：校验 HTTP 状态码、业务状态码、关键字段存在性
- **环境切换**：环境变量管理多套配置
- **代码级实现**：Python + requests 二次封装，体现编程能力
- **CI 设计**：提供可落地的持续集成方案
